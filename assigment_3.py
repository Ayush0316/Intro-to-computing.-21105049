...
# inistalizing the printing string.
starting_string = "ABCDEFGHIJK"

# calculating the length of the string.
length = len(starting_string)

# Using for loop to print the strings.
for counters in range(int(length/2) + 1):
    print(" "*counters + starting_string)

    # Removing last two letters from the string if possible.
    if length >= 2:
        starting_string = starting_string.rstrip(starting_string[-1])
        starting_string = starting_string.rstrip(starting_string[-1])
        length -= 2
